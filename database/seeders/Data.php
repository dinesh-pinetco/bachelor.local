<?php

namespace Database\Seeders;

use App\Models\Field;

class Data
{
    public static function getTabTableData()
    {
        $data = [
            [
                'name'        => 'Profil',
                'description' => 'Persönliche Informationen',
                'slug'        => 'profile',
                'icon'        => 'profile',
                'sort_order'  => 1,
                'meta_data'   => [
                    'button_text' => 'speichern',
                ],
                'groups'      => [
                    [
                        'internal_name' => __('Personal Information'),
                        'title'         => '',
                        'description'   => '',
                        'fields'        => [
                            [
                                'type'        => Field::FIELD_FILE,
                                'label'       => 'Bewerbungsfoto',
                                'placeholder' => 'Wählen Sie eine Datei',
                                'key'         => 'avatar',
                                'sort_order'  => 1,
                                'is_active'   => 1,
                                'is_required' => 1,
                                'meta_data'   => [],
                            ],
                            [
                                'type'        => Field::FIELD_SELECT,
                                'label'       => 'Geschlecht',
                                'placeholder' => 'Geschlecht auswählen',
                                'key'         => 'gender',
                                'sort_order'  => 2,
                                'is_active'   => 1,
                                'is_required' => 1,
                                'options'     => [
                                    [
                                        'key'   => 'male',
                                        'value' => 'Männlich',
                                    ],
                                    [
                                        'key'   => 'female',
                                        'value' => 'Weiblich',
                                    ],
                                    [
                                        'key'   => 'other',
                                        'value' => 'Divers',
                                    ],
                                ],
                                'meta_data'   => [],
                            ],
                            [
                                'type'        => Field::FIELD_TEXT,
                                'label'       => 'Vorname',
                                'key'         => 'first_name',
                                'placeholder' => 'Nachnamen eingeben',
                                'sort_order'  => 1,
                                'is_active'   => 1,
                                'is_required' => 1,
                                'meta_data'   => [],
                            ],
                            [
                                'type'        => Field::FIELD_TEXT,
                                'label'       => 'Nachname',
                                'key'         => 'last_name',
                                'placeholder' => 'Nachnamen eingeben',
                                'sort_order'  => 1,
                                'is_active'   => 1,
                                'is_required' => 1,
                                'meta_data'   => [],
                            ],
                            [
                                'type'        => Field::FIELD_SELECT,
                                'label'       => 'Akademischer Grad/Titel',
                                'placeholder' => 'Akademischer Grad/Titel auswählen',
                                'key'         => 'academic_degree',
                                'sort_order'  => 1,
                                'is_active'   => 1,
                                'is_required' => 1,
                                'options'     => [
                                    [
                                        'key'   => 'Bachelor',
                                        'value' => 'Bachelor',
                                    ],
                                    [
                                        'key'   => 'Master',
                                        'value' => 'Master',
                                    ],
                                    [
                                        'key'   => 'Doktor',
                                        'value' => 'Doktor',
                                    ],
                                ],
                                'meta_data'   => [],
                            ],
                            [
                                'type'        => Field::FIELD_DATE,
                                'label'       => 'Geburtsdatum',
                                'placeholder' => '',
                                'key'         => 'date_of_birth',
                                'sort_order'  => 1,
                                'is_active'   => 1,
                                'is_required' => 1,
                                'meta_data'   => [],
                            ],
                            [
                                'type'        => Field::FIELD_TEXT,
                                'label'       => 'Geburtsort',
                                'placeholder' => 'Geburtsort eingeben',
                                'key'         => 'place_of_birth',
                                'sort_order'  => 1,
                                'is_active'   => 1,
                                'is_required' => 1,
                                'meta_data'   => [],
                            ],
                            // [
                            //     'type'        => Field::FIELD_TEXT,
                            //     'label'       => 'Geburtsland',
                            //     'placeholder' => 'Geburtsland eingeben',
                            //     'sort_order'  => 1,
                            //     'is_active'   => 1,
                            //     'is_required'   => 1,
                            //     'meta_data'   => [],
                            // ],
                            [
                                'type'                 => Field::FIELD_SELECT,
                                'label'                => 'Geburtsland',
                                'key'                  => 'nationality_id',
                                'placeholder'          => 'Geburtsland eingeben',
                                'related_option_table' => 'nationalities',
                                'sort_order'           => 1,
                                'is_active'            => 1,
                                'is_required'          => 1,
                                'options'              => [],
                                'meta_data'            => [],
                            ],
                            [
                                'type'                 => Field::FIELD_SELECT,
                                'label'                => 'Staatsbürgerschaft (Staat)',
                                'key'                  => 'citizenship_id',
                                'placeholder'          => 'Staatsangehörigkeit eingeben (Staat)',
                                'related_option_table' => 'nationalities',
                                'sort_order'           => 1,
                                'is_active'            => 1,
                                'is_required'          => 1,
                                'options'              => [],
                                'meta_data'            => [],
                            ],
                        ],
                    ],
                    [
                        'internal_name' => __('Application for the'),
                        'title'         => 'Bewerbung für',
                        'description'   => '',
                        'fields'        => [
                            [
                                'type'                 => Field::FIELD_SELECT,
                                'label'                => 'Gewünschter Studiengang',
                                'key'                  => 'course_id',
                                'placeholder'          => 'Gewünschten Studiengang auswählen',
                                'sort_order'           => 1,
                                'related_option_table' => 'courses',
                                'is_active'            => 1,
                                'is_required'          => 1,
                                'options'              => [],
                                'meta_data'            => [],
                            ],
                            [
                                'type'                 => Field::FIELD_SELECT,
                                'label'                => 'Zum gewünschten Studienbeginn',
                                'key'                  => 'desired_beginning_id',
                                'placeholder'          => 'Wählen Sie Gewünschter Studienbeginn',
                                'sort_order'           => 2,
                                'related_option_table' => 'desired_beginnings',
                                'is_active'            => 1,
                                'is_required'          => 1,
                                'options'              => [],
                                'meta_data'            => [],
                            ],
                        ],
                    ],
                    [
                        'internal_name' => __('Contact Details'),
                        'title'         => 'Kontaktdetails',
                        'description'   => '',
                        'fields'        => [
                            [
                                'type'        => Field::FIELD_TEXT,
                                'label'       => 'Straße/Hausnummer',
                                'placeholder' => 'Straße und Hausnummer eingeben',
                                'key'         => 'street_house_number',
                                'sort_order'  => 1,
                                'is_active'   => 1,
                                'is_required' => 1,
                                'meta_data'   => [],
                            ],
                            [
                                'type'        => Field::FIELD_TEXT,
                                'label'       => 'Postleitzahl',
                                'placeholder' => 'Postleitzahl eingeben',
                                'key'         => 'postal_code',
                                'sort_order'  => 2,
                                'is_active'   => 1,
                                'is_required' => 1,
                                'meta_data'   => [],
                            ],
                            [
                                'type'        => Field::FIELD_TEXT,
                                'label'       => 'Standort',
                                'placeholder' => 'Ort eingeben',
                                'key'         => 'location',
                                'sort_order'  => 3,
                                'is_active'   => 1,
                                'is_required' => 1,
                                'meta_data'   => [],
                            ],
                            [
                                'type'        => Field::FIELD_TEL,
                                'label'       => 'Telefon',
                                'placeholder' => 'Telefon eingeben',
                                'key'         => 'phone',
                                'sort_order'  => 4,
                                'is_active'   => 1,
                                'is_required' => 1,
                                'meta_data'   => [],
                            ],
                            [
                                'type'        => Field::FIELD_EMAIL,
                                'label'       => 'E-Mail Adresse',
                                'placeholder' => 'E-Mail Adresse eingeben',
                                'key'         => 'email',
                                'sort_order'  => 5,
                                'is_active'   => 1,
                                'is_required' => 1,
                                'meta_data'   => [],
                            ],
                            [
                                'type'        => Field::FIELD_CHECKBOX,
                                'label'       => '',
                                'placeholder' => null,
                                'key'         => 'privacy_policy',
                                'sort_order'  => 6,
                                'is_active'   => 1,
                                'is_required' => 1,
                                'meta_data'   => [],
                                'options'     => [
                                    [
                                        'key'   => '1',
                                        'value' => '<div>Ich habe die Datenschutzbestimmungen <a href="https://www.nordakademie.de/datenschutz" class="underline text-blue-600">gelesen</a> und ich bin damit einverstanden.</div>',
                                    ],
                                ],
                            ],
                        ],
                    ],
                ],
            ],
            [
                'name'        => 'Werdegang',
                'description' => 'Kurzüberblick über Ihre beruflichen Tätigkeiten',
                'slug'        => 'career',
                'icon'        => 'career',
                'sort_order'  => 2,
                'meta_data'   => [
                    'button_text' => 'Speichern und weiter',
                ],
                'groups'      => [
                    [
                        'internal_name'  => 'Un 1',
                        'title'          => '',
                        'description'    => '<p>Bitte geben Sie einen kurzen Überblick über Ihre berufliche Laufbahn. Bitte beginnen Sie mit Ihrer gegenwärtigen Position. Bei Bedarf fügen Sie bitte weitere Zeilen ein.</p> <p> Wenn bei Ende kein Zeitpunkt eingegeben wird, wird angenommen, dass Sie die Stelle zur Zeit ausüben</p> <p><b>Sollten Sie bisher keine Arbeit ausgeübt haben, schreiben Sie bitte als Arbeitgeber “keinen” und lassen den Zeitraum offen.</b></p>',
                        'can_add_more'   => true,
                        'add_more_label' => 'weiteren Arbeitgeber hinzufügen',
                        'groups'         => [
                            [
                                'internal_name' => 'Academic career to date',
                                'title'         => 'Bisherige akademische Laufbahn',
                                'description'   => '',
                                'fields'        => [
                                    [
                                        'type'        => Field::FIELD_TEXT,
                                        'label'       => 'Name des Unternehmens',
                                        'placeholder' => 'Firmenname eingeben',
                                        'sort_order'  => 1,
                                        'is_active'   => 1,
                                        'is_required' => 1,
                                        'meta_data'   => [],
                                    ],
                                ],
                            ],
                            [
                                'internal_name' => 'Period of activity',
                                'title'         => 'Zeitraum der Tätigkeit',
                                'description'   => '',
                                'fields'        => [
                                    [
                                        'type'        => Field::FIELD_MONTH_YEAR,
                                        'label'       => 'Beginn',
                                        'placeholder' => null,
                                        'sort_order'  => 1,
                                        'is_active'   => 1,
                                        'is_required' => 1,
                                        'meta_data'   => [],
                                    ],
                                    [
                                        'type'        => Field::FIELD_MONTH_YEAR,
                                        'label'       => 'Ende',
                                        'placeholder' => null,
                                        'sort_order'  => 2,
                                        'is_active'   => 1,
                                        'is_required' => 1,
                                        'meta_data'   => [],
                                    ],
                                ],
                            ],
                        ],
                    ],
                    [
                        'internal_name'  => 'un121',
                        'title'          => '',
                        'description'    => '<p>Bitte geben Sie einen kurzen Überblick über Ihre Hochschulausbildung, beginnend mit der zuletzt besuchten Institution.</p> <p>Sollte Ihr Erststudium noch nicht abgeschlossen sein, geben Sie bitte den angestrebten Abschluss an.</p>',
                        'can_add_more'   => true,
                        'add_more_label' => 'weitere Hochschulbildung hinzufügen',
                        'groups'         => [
                            'internal_name' => 'Academic career to date',
                            'title'         => 'Bisherige akademische Laufbahn',
                            'description'   => '',
                            'fields'        => [
                                [
                                    'type'                 => Field::FIELD_SELECT,
                                    'label'                => 'Name der Hochschule',
                                    'placeholder'          => 'Name eingeben',
                                    'related_option_table' => 'universities',
                                    'key'                  => 'university',
                                    'sort_order'           => 1,
                                    'is_active'            => 1,
                                    'is_required'          => 1,
                                    'options'              => [],
                                    'meta_data'            => [],
                                ],
                                [
                                    'type'        => Field::FIELD_TEXT,
                                    'label'       => 'Studiengang eingeben',
                                    'placeholder' => null,
                                    'sort_order'  => 2,
                                    'is_active'   => 1,
                                    'is_required' => 1,
                                    'options'     => [],
                                    'meta_data'   => [],
                                ],
                                [
                                    'type'                 => Field::FIELD_SELECT,
                                    'label'                => 'Erworbener Abschluss',
                                    'placeholder'          => 'Bitte wählen',
                                    'related_option_table' => 'courses',
                                    'sort_order'           => 2,
                                    'is_active'            => 1,
                                    'is_required'          => 1,
                                    'options'              => [],
                                    'meta_data'            => [],
                                ],
                                [
                                    'type'        => Field::FIELD_TEXT,
                                    'label'       => 'ECTS-Kreditpunkte',
                                    'placeholder' => 'Punkte eingeben',
                                    'key'         => 'ects_point',
                                    'sort_order'  => 2,
                                    'is_active'   => 1,
                                    'is_required' => 1,
                                    'options'     => [],
                                    'meta_data'   => [],
                                ],
                                [
                                    'type'        => Field::FIELD_MONTH,
                                    'label'       => 'Monat/Jahr des Studienabschlusses',
                                    'placeholder' => '',
                                    'sort_order'  => 2,
                                    'is_active'   => 1,
                                    'is_required' => 1,
                                    'options'     => [],
                                    'meta_data'   => [],
                                ],
                            ],
                        ],

                    ],
                    [
                        'internal_name' => 'Un 2',
                        'title'         => '',
                        'description'   => 'Bitte geben Sie die Abschluss-/Durchschnittsnote des höchsten Bildungsabschlusses ein. Sollte Ihr Erststudium noch nicht abgeschlossen sein, geben Sie bitte die aktuelle Durchschnittsnote ein.',
                        'fields'        => [
                            [
                                'type'        => Field::FIELD_NUMBER,
                                'label'       => 'Hinweis',
                                'placeholder' => 'Notiz / Nummer eingeben',
                                'sort_order'  => 1,
                                'key'         => 'grade',
                                'is_active'   => 1,
                                'is_required' => 1,
                                'options'     => [],
                                'meta_data'   => [],
                            ],
                        ],
                    ],
                ],

            ],
            [
                'name'        => 'Motivation',
                'description' => 'Erzählen Sie etwas über sich',
                'slug'        => 'motivation',
                'icon'        => 'motivation',
                'sort_order'  => 3,
                'meta_data'   => [
                    'button_text' => 'Speichern und weiter',
                ],
                'groups'      => [
                    [
                        'internal_name' => 'Motivations',
                        'title'         => '',
                        'description'   => '',
                        'fields'        => [
                            [
                                'type'        => Field::FIELD_TEXTAREA,
                                'label'       => 'Bitte begründen Sie Ihren Studienwunsch.',
                                'placeholder' => '',
                                'sort_order'  => 1,
                                'is_active'   => 1,
                                'is_required' => 1,
                                'options'     => [],
                                'meta_data'   => [],
                            ],
                            [
                                'type'        => Field::FIELD_TEXTAREA,
                                'label'       => 'Welche Erwartungen haben Sie an das Studium (Inhalte, Studienmotivation, Berufsplanung, zeitlicher und finanzieller Aufwand)?',
                                'placeholder' => '',
                                'sort_order'  => 1,
                                'is_active'   => 1,
                                'is_required' => 1,
                                'meta_data'   => [],
                            ],
                            [
                                'type'        => Field::FIELD_TEXTAREA,
                                'label'       => 'Unterstützt Ihr Arbeitgeber Sie zeitlich und/oder finanziell?',
                                'placeholder' => '',
                                'sort_order'  => 1,
                                'is_active'   => 1,
                                'is_required' => 1,
                                'meta_data'   => [],
                            ],
                        ],
                    ],
                ],
            ],
        ];

        return collect($data);
    }
}
