...
Cypress.Commands.add('refreshRoutes', () => {
    return cy.csrfToken().then((token) => {
        return cy
            ...
            .its('body', {log: false})
            .then((routes) => {
                cy.writeFile('tests/cypress/support/routes.json', routes, {
                    log: false,
                });
 ...
 * @param {Object} attributes
 *
 * @example cy.create('App\\Models\\User');
 *          cy.create('App\\Models\\User', 2);
 *          cy.create('App\\Models\\User', 2, { active: false });
 *          cy.create('App\\Models\\User', { active: false });
 *          cy.create('App\\Models\\User', 2, { active: false });
 *          cy.create('App\\Models\\User', 2, { active: false }, ['profile']);
 *          cy.create('App\\Models\\User', { active: false }, ['profile']);
 *          cy.create('App\\Models\\User', ['profile']);
 */
...
Cypress.on('uncaught:exception', (err, runnable) => {
    if (expect(err.message).to.include('Livewire Sortable Plugin: window.livewire is undefined'))
        return false
    else {
        debugger
        return true
