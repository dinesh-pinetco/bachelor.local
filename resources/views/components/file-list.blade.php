@props(['media','isEdit'=>false])
<div wire:key="{{ $media->id }}" class="max-w-4xl px-4 md:px-6 lg:px-8 py-3 bg-lightgray rounded-sm flex items-center text-base">
    <div class="flex-grow w-1/3 pr-4">
        <a href="{{ $media->url }}" target="_blank" class="block w-full truncate font-bold">{{ $media->name }}</a>
        <div class="form-check">
            <input id="flexCheckChecked" wire:model="media.is_check" wire:change="handleIsCheck($event.target.value)"
    ...
    <button @if ($isEdit)  wire:click.prevent="delete('{{ $media->id }}')"  @endif  type="button" class="py-2 flex-shrink-0 inline-block">
