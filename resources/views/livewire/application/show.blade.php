        ...
        <div class="flex-grow w-1/3 text-primary relative">
            <div class="md:flex items-center justify-between {{ $isProfile ? 'my-7 lg:my-10' : '' }}">
                ...
                <div class="flex items-center space-x-4 flex-shrink-0">
                    ...
                    <div class="inline-flex items-center gap-2 text-green-500 text-sm mt-4 md:mt-0">
                        <svg class="w-3 h-3 fill-current" fill="currentcolor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                    ...
                    <div class="flex items-center space-x-4">
                        <x-primary-button type="button" wire:click="$emit('Applicant.Modal.Enrollment.modal.toggle',{{ $applicant->id }})" :disabled="!$isEdit" class="md:-mt-0 {{ $isEdit ? 'cursor-pointer' : 'cursor-not-allowed' }}">
                            {{ __('Enroll Applicant') }}
                        </x-primary-button>
                    </div>
                    @endif
                ...
                <div>
                    <div class="flex items-center justify-end space-x-2 mb-5 md:mb-0">
                        ...
                        <div>
                            <a class="w-10 h-10 flex items-center justify-center bg-primary hover:bg-opacity-80 rounded-sm" href="industries">
                                <svg class="w-4 h-4 stroke-current text-white flex-shrink-0" width="25" height="25" viewBox="0 0 24 24" stroke-width="2.50" fill="none" xmlns="http://www.w3.org/2000/svg" color="#003A79">
                        ...
                        <div>
                            <a class="w-10 h-10 flex items-center justify-center bg-primary hover:bg-opacity-80 rounded-sm" href="{{ route('documents.index') }}">
                                <svg class="w-4 h-4 stroke-current text-white flex-shrink-0" width="25" height="25" viewBox="0 0 24 24" stroke-width="2.50" fill="none" xmlns="http://www.w3.org/2000/svg" color="currentcolor">
                                    <path d="M3 12h18m0 0l-8.5-8.5M21 12l-8.5 8.5" stroke="currentcolor" stroke-linecap="round" stroke-linejoin="round">
                                    </path>
                                </svg>
                            </a>
                        </div>
            ...
            <div class="lg:flex mb-10 lg:-mx-4" id="group-key-{{ $customGroupKey }}" wire:key="{{ $customGroupKey }}">
                <div class="w-full lg:w-1/2 lg:px-4 lg:max-w-md mr-auto">
                    @if ($group->title)
                    <label class="mb-4 text-xl md:text-3xl font-bold text-primary" for="">{{ $group->title }}</label>
                    @endif
                    <div class="w-full space-y-8">
                        ...
                        <div class="value" wire:key="{{ $fieldKey }}">
                            <livewire:field :applicant="$applicant" :isEdit="$isEdit" :groupKey="$customGroupKey" :value="$value" :key="time() . $value->id" />
                            <x-jet-input-error for="field.{{ $field['key'] }}" class="mt-2" />
                        </div>
                        @empty
                        <div class="field" wire:key="{{ $fieldKey }}">
                            <livewire:field :applicant="$applicant" :isEdit="$isEdit" :groupKey="$customGroupKey" :field="$field" :key="time() . $field->id" />
                        </div>
                        @endforelse
                        @else
                        <div class="field" wire:key="{{ $fieldKey }}">
                        ...
                        <div class="space-y-8" wire:key="{{ $child->id }}">
                            ...
                            <div class="mb-7">
                                ...
                                <div class="mb-7 field" wire:key="{{ $childFieldKey }}">
                                    <livewire:field :applicant="$applicant" :isEdit="$isEdit" :groupKey="$customGroupKey" :field="$childField" :key="time() . $childField->id" />
                                </div>
                                @endforelse
                                @else
                                <div class="mb-7 field" wire:key="{{ $childFieldKey }}">
                ...
                <div class="w-full lg:w-1/2 lg:px-4 lg:max-w-md mr-auto">
                    ...
                    <div class="flex justify-start items-baseline mb-7">
                        <div class="form-check">
                            <input wire:model="competency_catch_up" @if ($isEdit) wire:change="handleCompetencyCatchUp" @else {{ "disabled" }} @endif class="flex-shrink-0 w-5 h-5 mt-1 text-primary form-checkbox focus:border-primary-light focus:ring focus:ring-primary-light focus:ring-opacity-50 shadow-sm outline-none" type="checkbox" id="competency_catch_up">
                    ...
                    <div>
                        ...
                        <div wire:ignore>
                            <trix-editor class="prose formatted-content" x-data x-on:trix-change="$dispatch('input', event.target.value)" x-ref="trix" wire:model.debounce.500ms="competency_comment" wire:key="competency_comment"></trix-editor>
                        </div>
