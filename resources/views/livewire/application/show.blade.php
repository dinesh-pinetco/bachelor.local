                            ...
                            <div class="w-full lg:w-1/2 lg:px-4 lg:max-w-md mr-auto">
                                ...
                                @if ($group->can_add_more && $isEdit)
                                    @if ($loop->first != $loop->last || !$loop->first)
                                        <x-danger-button class="group-remove" group_key="{{ $customGroupKey }}" wire:click="removeGroup({{ $customGroupKey }})" wire:loading.attr="disabled">
                                            {{ __('Remove') }}
                                        </x-danger-button>
                                    @endif
                                    @if ($loop->last)
                                        <x-primary-button wire:click="appendGroup({{ $group->id }})" wire:loading.attr="disabled">
