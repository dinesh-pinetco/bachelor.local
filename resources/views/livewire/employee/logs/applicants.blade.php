    ...
    <x-data-table.table :model="$audits" :columns="$columns">
        ...
        <x-slot name="head">
            <tr>
                ...
                <th scope="col" class="px-6 py-3 text-left text-sm font-medium text-darkgray tracking-wider">
                    {{ __('Field') }} </th>
                <th scope="col" class="px-6 py-3 text-center text-sm font-medium text-darkgray tracking-wider">
                    {{ __('View') }} </th>
            </tr>
        </x-slot>
        <x-slot name="body">
            @forelse ($audits as $audit)
                <tr wire:key="field-{{ $audit->id }}">
                    ...
                    <td class="px-6 py-4 text-sm text-primary">
                        <div class="w-full max-w-xs break-normal">
                            {{ class_basename($audit->auditable) }}
                        </div>
                    </td>
                    <td class="px-6 py-4 text-sm text-primary">
                        <div class="w-full max-w-xs break-normal">
                            {{ $audit->auditable && $audit->auditable->fields ? $audit->auditable->fields->label : '' }}
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-primary text-center">
