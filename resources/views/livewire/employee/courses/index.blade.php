<div>
    <x-data-table.table :model="$courses" :columns="$columns">
        <x-slot name="tableAction">
            <x-link-button class="flex items-center mb-4 xl:mb-0 -mt-0" href="{{ route('employee.courses.create') }}">
        ...
        <x-slot name="head">
            <tr>
                ...
                <th scope="col" class="px-6 py-3 text-left text-sm font-medium text-darkgray tracking-wider">
                    <a wire:click="sort('form_of_study')" role="button" href="#">
                        {{ __('Form of study') }}
                        @include('components.data-table.sort-icon', ['field' => 'form_of_study'])
                    </a>
                </th>
        ...
        <x-slot name="body">
            @forelse ($courses as $course)
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-primary">{{ $course->name }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-primary">{{ $course->form_of_study }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-primary">{{ $course->isActiveLabel }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                        <a data-cy="edit-button-{{ $course->id }}" role="button"
                            class="text-darkgray hover:text-gray inline-block cursor-pointer"
                            href="{{ route('employee.courses.edit', $course->id) }}">
                        ...
                        <a data-cy="clone-button-{{ $course->id }}" role="button"
                            class="text-darkgray hover:text-primary inline-block cursor-pointer"
                            href="{{ route('employee.courses.clone', $course->id) }}">
                            <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current h-4 w-4" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <a data-cy="delete-button-{{ $course->id }}" role="button"
                            class="text-darkgray hover:text-lightred inline-block cursor-pointer"
                            wire:click="openConfirmModal({{ $course->id }})" href="#">
                            <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current h-4 w-4" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div>
            <div class="space-y-3">
                ...
                <h4 class="text-center text-darkgray text-sm sm:text-base">
                    {{ __('Are you sure you want to remove course') }}?
                </h4>
            </div>
        </div>
