        ...
        <div class="flex flex-wrap flex-grow -mx-4">
            <div class="p-4 w-full shadow-md rounded">
                ...
                <div class="flex items-center justify-between flex-wrap">
                    ...
                    <button @if($isEdit) wire:click="markAsPassed" @else disabled @endif type="button" class = "items-center -mt-0 inline-block px-4 py-2 bg-primary border border-transparent rounded-sm font-semibold text-base text-white hover:bg-opacity-90 focus:outline-none focus:bg-opacity-90 disabled:opacity-25 transition mt-4 duration-150 ease-in-out {{ $isEdit ? 'cursor-pointer' : 'cursor-not-allowed' }}">
                        {{ __('Mark test as passed') }}
                    </button>
                </div>
                <div class="flex items-center justify-between flex-wrap mt-3 mb-3 space-x-2">
                    ...
                    <button @if($isEdit) wire:click="markAsReset" @else disabled @endif type="button" class = "items-center -mt-0 inline-block px-4 py-2 bg-primary border border-transparent rounded-sm font-semibold text-base text-white hover:bg-opacity-90 focus:outline-none focus:bg-opacity-90 disabled:opacity-25 transition mt-4 duration-150 ease-in-out {{ $isEdit ? 'cursor-pointer' : 'cursor-not-allowed' }}">
                        {{ __('Reset') }}
                    </button>
                </div>
            </div>
