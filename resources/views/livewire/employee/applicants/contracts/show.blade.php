        ...
        <div class="flex-grow w-1/3 text-primary relative">
            <div class="lg:flex mb-10 lg:-mx-4">
                <div class="w-full lg:w-1/2 lg:px-4 lg:max-w-md mr-auto">
                    @if ($applicant->application_status_id >= User::STATUS_SELECTION_INTERVIEW_ON && $applicant->application_status_id < User::STATUS_APPLICATION_REJECTED_BY_NAK)
                        <div class="mb-7 value">
                            <x-jet-label for="label" class="block">
                                {{ __('Contract Send Date') }}
                            </x-jet-label>
                            <div class="flex">
                                <x-jet-input min="{{ $applicant->interview->date }}" disabled="{{ !$isEdit }}"
                                    class="w-full {{ $isEdit ? 'text-primary' : 'text-gray cursor-not-allowed' }}" type="date" wire:model="contract.send_date">
                                </x-jet-input>
                        ...
                        @if ($applicant->application_status_id >= User::STATUS_CONTRACT_SENT_ON && $applicant->application_status_id < User::STATUS_APPLICATION_REJECTED_BY_NAK)
                            <div class="mb-7 value">
                                <x-jet-label for="label" class="block">
                                ...
                                <div class="flex">
                                    <x-jet-input min="{{ $contract->send_date }}" disabled="{{ !$isEdit }}"
                                        class="w-full {{ $isEdit ? 'text-primary' : 'text-gray cursor-not-allowed' }}" type="date" wire:model="contract.receive_date">
                                    </x-jet-input>
