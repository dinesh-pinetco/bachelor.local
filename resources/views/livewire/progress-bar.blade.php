<div class="h-full">
    <div :class="{ 'bg-primary hover:bg-opacity-80 text-white': '{{ $overAllProgress == 100 }}' }"
        ...
        @if ($overAllProgress == 100)
            <button wire:click="$set('show', true)" :class="{ 'opacity-100': '{{ $overAllProgress == 100 }}' }"
                       class="hidden md:inline-block text-sm lg:text-base opacity-0 select-none text-white font-bold capitalize mr-6 -mb-0">{{ __('Submit') }}</button>
        @endif
