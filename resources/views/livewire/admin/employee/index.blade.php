<div>
    <x-data-table.table :model="$users" :columns="$columns">
        <x-slot name="tableAction">
            <x-link-button class="flex items-center mb-4 xl:mb-0 -mt-0" href="{{ route('admin.employees.create') }}">
            ...
            <tr>
                <th scope="col" class="px-6 py-3 text-left text-sm font-medium text-darkgray tracking-wider">
                    <a wire:click="sort('first_name')" role="button" href="#">
                        {{ __('First name') }}
                        @include('components.data-table.sort-icon', ['field' => 'first_name'])
                    </a>
                </th>
                <th scope="col" class="px-6 py-3 text-left text-sm font-medium text-darkgray tracking-wider">
                    <a wire:click="sort('last_name')" role="button" href="#">
                        {{ __('Last name') }}
                        @include('components.data-table.sort-icon', ['field' => 'last_name'])
                    </a>
                </th>
                <th scope="col" class="px-6 py-3 text-left text-sm font-medium text-darkgray tracking-wider">
                    <a wire:click="sort('email')" role="button" href="#">
                        {{ __('Email') }}
                        @include('components.data-table.sort-icon', ['field' => 'email'])
                    </a>
                </th>

                <th scope="col" class="px-6 py-3 text-left text-sm font-medium text-darkgray tracking-wider">
                    <a wire:click="sort('phone')" role="button" href="#">
                        {{ __('Phone number') }}
                        @include('components.data-table.sort-icon', ['field' => 'phone'])
                    </a>
                </th>
        ...
        <x-slot name="body">
            @forelse ($users as  $user)
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-primary">{{ $user->first_name }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-primary">{{ $user->last_name }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-primary">{{ $user->email }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-primary">{{ $user->phone }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                        <a data-cy="edit-button-{{ $user->id }}" role="button"
                        ...
                        <a data-cy="delete-button-{{ $user->id }}" role="button"
                            class="text-darkgray hover:text-lightred inline-block cursor-pointer"
                            wire:click="openConfirmModal({{ $user->id }})" href="#">
                            <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current h-4 w-4" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div>
            <div class="space-y-3">
                ...
                <h4 class="text-center text-darkgray text-sm sm:text-base">
                    {{ __('Are you sure you want to remove employee') }}?
                </h4>
            </div>
        </div>
